...
async function ensureAccess(clientId) {
  ...
  // If token expired and refresh_token present, attempt refresh (basic check)
...
// Helper to call Google Sheets API with auth
...
export async function appendTiming(spreadsheetId, entry, clientId) {
  ...
  // try to infer new row id from updates.updatedRange
...
export async function getSheetLinks(spreadsheetId, clientId) {
  // Try to get spreadsheet metadata to construct links
...
export async function createSpreadsheetIfMissing(spreadsheetId, clientId) {
  // If spreadsheetId provided, assume exists
